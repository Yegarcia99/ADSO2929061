    ...
    <main class="add">
            ...
            <input type="text" name="name" placeholder="Nombre"
                ...
                <div class="select">
                    ...
                    <?php foreach ($species as $specie): ?>
                        <option value="<?= $specie['id'] ?>" <?php if (isset($_POST['specie_id']) && $_POST['specie_id'] == $specie['id'])
                            echo ("selected") ?>><?= $specie['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            <div class="select">
                <select name="breed_id">
                    ...
                    <?php foreach ($breeds as $breed): ?>
                        <option value="<?= $breed['id'] ?>" <?php if (isset($_POST['breed_id']) && $_POST['breed_id'] == $breed['id'])
                            echo ("selected") ?>><?= $breed['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            ...
            <div class="select">
                <select name="sex_id">
                    ...
                    <?php foreach ($sexes as $sex): ?>
                        <option value="<?= $sex['id'] ?>" <?php if (isset($_POST['sex_id']) && $_POST['sex_id'] == $sex['id'])
                            echo ("selected") ?>><?= $sex['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
        ...
        if ($_POST) {
            ...
            if ($errors == 0) {
                ...
                $sex_id = $_POST['sex_id'];
                $photo = time() .'.'.pathinfo($_FILES
                ['photo']['name'], PATHINFO_EXTENSION);
